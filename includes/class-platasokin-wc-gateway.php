<?php
/**
 * WooCommerce payment gateway and related hooks.
 *
 * @package Platasokin
 */
...
add_action( 'woocommerce_refund_created', 'platasokin_create_refund', 10, 2 );

/**
 * Refund handler for Sokin Pay orders.
 *
 * @param int   $refund_id Refund post ID.
 * @param array $args      Refund arguments including amount and reason.
 */
function platasokin_create_refund( $refund_id, $args ) {
	...
	if ( $order->get_meta( 'orderId' ) ) {
		$order_args = array(
			'headers' => array(
				'x-api-key'    => $payment_gateway->settings['platasokin_x_api_key'],
				'Content-Type' => 'application/json',
			),
		);

		$order_request_url = $payment_gateway->platasokin_api_url . '/orders/' . $order->get_meta( 'orderId' );
		...
		$response = wp_remote_post( $payment_gateway->platasokin_api_url . '/refunds', $header_args );
 ...
 */
function platasokin_gateway_log_warning( $message, $context = array() ) {
	$source = isset( $context['source'] ) ? $context['source'] : 'platasokin-gateway';
...
add_action( 'woocommerce_thankyou', 'platasokin_thankyou_handler', 10, 1 );

/**
 * Thank-you page handler for Sokin Pay orders.
 *
 * @param int $order_id WooCommerce order ID.
 */
function platasokin_thankyou_handler( $order_id ) {
...
add_filter( 'woocommerce_payment_gateways', 'platasokin_add_gateway_class' );

/**
 * @param array<int, string> $gateways Gateway class names.
 * @return array<int, string>
 */
function platasokin_add_gateway_class( $gateways ) {
	$gateways[] = 'Platasokin_WC_Gateway';
	return $gateways;
}

add_action( 'plugins_loaded', 'platasokin_init_gateway_class' );

/**
 * Define payment gateway class after WooCommerce loads.
 */
function platasokin_init_gateway_class() {
	if ( ! class_exists( 'WC_Payment_Gateway' ) ) {
		return;
	}

	/**
	 * Sokin Pay WooCommerce gateway.
	 */
	class Platasokin_WC_Gateway extends WC_Payment_Gateway {

		/**
		 * @var string
		 */
		public $platasokin_redirect_url;

		/**
		 * @var string
		 */
		public $platasokin_x_api_key;

		/**
		 * @var string
		 */
		public $platasokin_api_url;

		public function __construct() {
			self::migrate_legacy_gateway_settings();

			$this->id                 = 'sokinpay_gateway';
			$this->icon               = plugin_dir_url( __FILE__ ) . '../assets/images/payment_methods.svg';
		...
		/**
		 * Merge legacy option keys into platasokin_* (and standard enabled) once.
		 */
		private static function migrate_legacy_gateway_settings() {
			$option_key = 'woocommerce_sokinpay_gateway_settings';
			...
			$key_map = array(
				'platasokin_redirect_url' => 'woo_cpay_redirect_url',
				'platasokin_x_api_key'    => 'woo_cpay_x_api_key',
				'platasokin_api_url'      => 'woo_cpay_api_url',
			);
		...
		public function init_form_fields() {
			$this->form_fields = array(
				'enabled'                 => array(
					'title'   => __( 'Enable/Disable', 'sokin-pay' ),
					'type'    => 'checkbox',
					'label'   => __( 'Enable Sokin Pay', 'sokin-pay' ),
					'default' => 'no',
				),
				'title'                   => array(
					'title'       => __( 'Title', 'sokin-pay' ),
					'type'        => 'text',
					'description' => __( 'Payment title that the customer will see on your checkout.', 'sokin-pay' ),
					'desc_tip'    => true,
				),
				'description'             => array(
				...
				'platasokin_redirect_url' => array(
					'title' => __( 'Checkout URL', 'sokin-pay' ),
					'type'  => 'text',
				),
				'platasokin_x_api_key'    => array(
					'title' => __( 'X API Key', 'sokin-pay' ),
					'type'  => 'text',
				),
				'platasokin_api_url'      => array(
					'title' => __( 'API URL', 'sokin-pay' ),
					'type'  => 'text',
				),
			);
		}
		...
		public function payment_scripts() {
			wp_register_script(
				'platasokin_checkout_gateway',
				plugins_url( 'platasokin-checkout-gateway.js', __FILE__ ),
				array( 'jquery' ),
				PLATASOKIN_VERSION,
				true
			);
			wp_enqueue_script( 'platasokin_checkout_gateway' );
		}
		...
		public function process_payment( $order_id ) {
			...
			$args = array(
				'headers' => array(
					'x-api-key'    => $this->settings['platasokin_x_api_key'],
					'Content-Type' => 'application/json',
				),
				'body'    => wp_json_encode( $body ),
			);

			$response     = wp_remote_post( $this->settings['platasokin_api_url'] . '/orders', $args );
			...
			if ( ! is_wp_error( $response ) && is_array( $responceData ) && isset( $responceData['corporateId'], $responceData['orderId'] ) ) {
				$redirect_url = $this->settings['platasokin_redirect_url'] . '/' . $responceData['corporateId'] . '/' . $responceData['orderId'];
